...
| 項目 | 詳細 |
|------|------|
| データソース | Finnhub API（Stock Candles エンドポイント） |
| 対象市場 | 東京証券取引所（取引所コード: `T`） |
| シンボル形式 | `{証券コード}.T`（例: `7203.T` = トヨタ） |
| データ種別 | 日足 OHLCV（始値・高値・安値・終値・出来高） |
| 取得期間 | 過去約1年分（無料プランの上限） |
| 認証 | ユーザーが自身の Finnhub API キーを入力 |
...
| 銘柄一覧 | Finnhub Symbol Search API による候補表示 |
...
| 設定項目 | 詳細 |
|----------|------|
| API キー入力 | Finnhub API キーの入力・保存（ローカルストレージ等は使用せず、セッション中のみ保持） |
...
| エラー種別 | 対処 |
|-----------|------|
| API キー未入力/無効 | 入力欄にエラーメッセージ表示 |
| 証券コードが存在しない | 「銘柄が見つかりません」表示 |
| API レート制限（429） | 「しばらく待ってから再試行してください」表示 |
...
| 項目 | 対策 |
|------|------|
| API キー管理 | ローカルストレージには保存しない。React state でセッション中のみ保持 |
| データ通信 | Finnhub API は HTTPS で通信 |
...
| 過去データ | 約1年分（日足） |
| リアルタイム性 | 15分遅延 |
| API キー | ユーザー各自で取得が必要（https://finnhub.io） |
